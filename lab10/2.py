...
def enter_user():
    ...
    cursor.execute(f"SELECT name FROM users_of_snake WHERE name like '{name}'")
    names = cursor.fetchall()
    try:
        if not names:
            cursor.execute(f"INSERT INTO users_of_snake (name) VALUES ('{name}')")
            conn.commit()
            print("User created successfully!")
        else:
            cursor.execute(f"SELECT id FROM users_of_snake WHERE name like '{name}'")
            user_id = cursor.fetchall()[0][0]
            cursor.execute(f"SELECT score FROM users_score WHERE users_id = {user_id}")

    except psycopg2.Error as e:
        print("Error occurred:", e)


def save_value(score):
    cursor.execute(f"INSERT INTO users_score(score, users_id) VALUES('{score}', '{user_id}')")
    conn.commit()
